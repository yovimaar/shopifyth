...
var elementClosest = __webpack_require__(748);
...
;// CONCATENATED MODULE: ./node_modules/@pixelunion/load-in-animations/dist/LoadInAnimations.js
...
// Find correct DOM elements using `mappingSelector` to add the 'block' and 'element' classes to.
const addClasses = (parentEl) => {
    parentEl
        .querySelectorAll(mappingSelector)
        .forEach(mappingEl => {
        const section = mappingEl.parentNode;
        try {
            const mapping = JSON.parse(mappingEl.innerHTML);
            mapping.elements.forEach((elementSelector) => {
                section
                    .querySelectorAll(elementSelector)
                    .forEach(element => element.classList.add(classes.element));
            });
            mapping.blocks.forEach((blockSelector) => {
                section
                    .querySelectorAll(blockSelector)
                    .forEach(block => block.classList.add(classes.block));
            });
        }
        catch (error) {
...
const getMappingParentFromBlock = ({ parentElement }) => (parentElement.querySelector(mappingSelector)
    ? parentElement
    : getMappingParentFromBlock(parentElement));
...
const intersectionCallback = (entries, observer) => {
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            toAnimate.set(parentSequence, existingSiblings ? [...existingSiblings, entry.target] : [entry.target]);
        }
    });
    toAnimate.forEach((blocks, _parent) => playLoadInAnimations(blocks));
};
// Remove blocks from the global intersection observer for automatic load-in animations.
const removeLoadInAnimationsAutoplay = (blocks) => (blocks.forEach(block => blockObserver === null || blockObserver === void 0 ? void 0 : blockObserver.unobserve(block)));
const initLoadInAnimationsAutoplay = (options = { threshold: 0.3 }, parentEl = document) => {
    if (!('reduceAnimations' in document.body.dataset)) {
        addClasses(parentEl);
        /*
          This assumes that animated elements are arranged by section, block, and element.
          Animations are triggered when blocks enter view,
          and are sequenced within sections by block and by subElement.
          Sequencing information is applied as custom properties that can be referenced in CSS.
        */
        blockObserver = new IntersectionObserver(intersectionCallback, options);
        const observe = (el) => {
            el
                .querySelectorAll(`.${classes.block}`)
                .forEach((block) => blockObserver === null || blockObserver === void 0 ? void 0 : blockObserver.observe(block));
        };
        observe(parentEl);
    }
};
...
function openShare(e) {
  e.stopPropagation();
  const targetShareWidget = e.target.closest('.article-meta-wrap');
  const otherShareWidgets = Array.prototype.filter.call(document.querySelectorAll('.article-meta-wrap'), el => el !== targetShareWidget);
  targetShareWidget.classList.add('active');
  otherShareWidgets.forEach(el => el.classList.remove('active'));
}

function closeShare(e) {
  if (!e.target.closest('.share-widget-toggle') && !e.target.closest('.share-widget')) {
    const articleMetaWrap = document.querySelector('.article-meta-wrap');
...
class Share {
  constructor(_ref) {
    ...
    this.onBodyClick = e => closeShare(e);

    this.onShareClick = e => openShare(e); //Init RTEs in articles


    this.rtes = [];
    this.el.querySelectorAll('[data-rich-text-area]').forEach(el => this.rtes.push(new Rte({
      el
    })));
    ...
    this.shareWidgetToggles.forEach(el => {
      el.addEventListener('click', this.onShareClick);
    });
  }
  ...
  unload() {
    ...
    if (this.shareWidgetToggles) {
      this.shareWidgetToggles.forEach(el => {
        el.removeEventListener('click', this.onShareClick);
      });
    }
  }
...
class Modal {
  constructor() {
    ...
    this.modal = null;
    this.events = new EventHandler/* default */.Z(); // Extend default vanilla-modal callbacks back to instantiator of Modal
    ...
    const modalSelector = this.options.modalId ? `[data-modal-container-${this.options.modalId}]` : '[data-modal-container]';
  ...
  open(selector) {
    let handle = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'general';
  ...
  position() {
    ...
    const modalHeight = this.modalInner.getBoundingClientRect().height;
    const modalPadding = parseInt(window.getComputedStyle(this.modalEl).paddingTop, 10) * 2;
    const offset = (windowHeight - modalPadding - modalHeight) / 2;
    this.modalInner.style.marginTop = offset > 0 ? `${offset}px` : 0;
  }
  /**
  ...
  _addModalClass(handle) {
    this.modalEl.classList.add(`modal--${handle}`);
  }
  /**
  ...
  _removeModalClass() {
    const modalClass = this.modalEl.className.match(/modal--[\w-]*\b/);
  ...
  _onOpen() {
    ...
    this.events.register(window, 'resize', just_debounce_default()(() => this.position(), 16, true, true));
    this.options.onOpen();
...
function shopify_variants_es_toConsumableArray(arr) {
  return shopify_variants_es_arrayWithoutHoles(arr) || shopify_variants_es_iterableToArray(arr) || shopify_variants_es_nonIterableSpread();
}

function shopify_variants_es_arrayWithoutHoles(arr) {
...
function shopify_variants_es_iterableToArray(iter) {
  if (Symbol.iterator in Object(iter) || Object.prototype.toString.call(iter) === "[object Arguments]") return Array.from(iter);
}

function shopify_variants_es_nonIterableSpread() {
  throw new TypeError("Invalid attempt to spread non-iterable instance");
}
...
class ProductDetails {
  constructor(options) {
    ...
    this.variantFields = {
      ...
      stockLevels: this.el.querySelectorAll('[data-inventory-status]')
    };
    ...
    if (this.variants) {
      ...
      this.variantHelper = new shopify_variants_es(this.product, this.variants, this.options, {
        onSwitchVariant: data => this._onSwitchVariant(data)
      });
    }

    const selectedVariant = () => this.variantHelper ? this.variantHelper.getSelectedVariant() : this.product.variants[0]; // product has only default variant or variant selector is hidden


    if (!this.variants) this._updateStockLevels(selectedVariant());
    ...
    const surfacePickUpEl = this.el.querySelector('[data-surface-pick-up]');
    ...
    if (surfacePickUpEl) {
      ...
      this.surfacePickUp.onModalRequest(contents => {
        let container = this.el.querySelector('[data-surface-pick-up-modal-contents]');
        const variantTitle = this.variantHelper ? `<div class="surface-pick-up-modal__variant">${selectedVariant().title}</div>` : '';
        const modalContents = `
  ...
  _updateStockLevels(variant) {
    this.variantFields.stockLevels.forEach(el => {
      const inventoryStatusVariantId = Number(el.dataset.inventoryStatusVariantId);
      el.setAttribute('data-inventory-status-variant-selected', `${variant.id === inventoryStatusVariantId}`);
    });
  }
  ...
  _updatePrice(variant) {
    ...
    if (!price || !atcButton) return;
    const variantPrice = Shopify.formatMoney(variant.price, window.Theme.moneyFormat);
    const variantComparePrice = Shopify.formatMoney(variant.compare_at_price, window.Theme.moneyFormat);
...
class ImageZoom {
  ...
  _bindEvents() {
    this.productZoomClick = this.events.register(this.el, 'click', event => this._toggleZoom(event.clientX, event.clientY));
  ...
  _layoutHandler() {
    const scale = this.img.naturalWidth / this.el.getBoundingClientRect().width;
    const zoomable = scale >= 1.25;
  ...
  _toggleZoom(clientX, clientY) {
    if (!this.zoomed) {
      this.zoomMouseMove = this.events.register(this.el, 'mousemove', ThemeUtils.throttle(event => this._positionZoom(event.clientX, event.clientY), 1000 / this.framerate));
  ...
  _positionZoom(clientX, clientY) {
    const figRect = this.el.getBoundingClientRect();
    ...
    this.img.style.transform = `scale(${this.scale}) translate(${translateX}px, ${translateY}px)`;
  }
...
class ProductGallery {
  constructor(el) {
    ...
    if (this.selected.figure) {
      const animationendCallback = event => {
        if (event.animationName === 'zoom' && event.elapsedTime > 0) {
          this.selected.figure.classList.remove('loaded-first');
          this.selected.figure.removeEventListener('animationend', animationendCallback);
        }
      };

      this.selected.figure.addEventListener('animationend', animationendCallback);
    }
    ...
    if (settings.enable_image_zoom) {
      const images = this.viewport.querySelectorAll('[data-media-type="image"]');
  ...
  _onModelLibraryLoad() {
    ...
    modelFigures.forEach(figure => {
      const el = figure.querySelector('model-viewer');
      const player = new Shopify.ModelViewerUI(el, {
        controls
      });
      const viewerControls = figure.querySelectorAll('.shopify-model-viewer-ui__button--control');
  ...
  _onExternalVideoLibraryLoad() {
    ...
    for (let i = 0; i < videoFigures.length; i++) {
      const videoEl = videoFigures[i];
      const player = new YT.Player(videoEl.querySelector('iframe'));
      const video = {
        type: 'video',
        figure: videoEl,
        player,
  ...
  _onMediaSelect(media) {
    const viewInYourSpaceEl = this.el.querySelector('[data-shopify-xr]');
...
class Product {
  constructor(section) {
    ...
    this.recipientForm = this.productForm.querySelector('[data-recipient-form]');
    ...
    this.gallery = new ProductGallery(this.el.querySelector('[data-product-gallery]'), {
    ...
    this.productDetails = new ProductDetails({
      ...
      settings: this.data,
      onVariantChange: (_ref) => {
        let {
          variant
        } = _ref;
        return this.gallery.selectMediaByVariant(variant);
      }
    });
    this.rtes = [];
    this.el.querySelectorAll('[data-rich-text-area]').forEach(el => this.rtes.push(new Rte({
    ...
    window.Shopify.onError = XMLHttpRequest => this.handleErrors(XMLHttpRequest);

    this.events.register(this.addToCartButton, 'click', event => this.addToCart(event));
    this.paymentTerms = new PaymentTerms(this.el);
  }

  addToCart(e) {
    if (this.data.enable_cart_redirection || document.querySelector('.template-cart')) {
      return;
    }

    e.preventDefault();
    const productMessageWrappers = this.el.querySelectorAll('.product-message');
    productMessageWrappers.forEach(el => el.parentNode.removeChild(el));
    if (this.addingToCart) return;
    const data = new FormData(this.productForm);
  ...
  handleErrors(error) {
    setTimeout(() => {
      this.addToCartButton.classList.remove('product-details__add-to-cart-button--loading');
      const productMessageWrapper = document.createElement('div');
      productMessageWrapper.classList.add('product-message', 'errors');
      const productAddToCart = this.el.querySelector('.product-details__add-to-cart-area');
      let errorMsg = this.data.generic_error_msg;

      if (error) {
        if (error.responseJSON) {
          errorMsg = error.responseJSON.description;
        } else if (error.message === "401") {
          errorMsg = `${errorMsg} ${this.data.error_msg_unauthorized}`;
        }
      }
      ...
      if (this.recipientForm) {
        ...
        productMessageWrapper.innerHTML = errorMsg;
        productAddToCart.appendChild(productMessageWrapper);
      }
...
class ProductQuickshop {
  ...
  open() {
    ...
    this.modal.open(`#${id} [data-product-quickshop-url="${url}"]`, 'quickshop', trigger);
    this.isOpen = true;
    loader.then(() => {
      ...
      initLoadInAnimationsAutoplay(null, this.el);
    });
  }
  ...
  load() {
    ...
    this.loader = shopify_asyncview_dist_index_es.load(url, {
      ...
      const targetContainer = this.isOpen ? this.modal.modalEl.querySelector('[data-modal-content]') : el;
      const spinner = targetContainer.querySelector('.quickshop__loading-state');
      spinner.parentNode.removeChild(spinner);
      this.el = targetContainer.appendChild(container);
...
const isQuickShopTrigger = el => 'quickshopTrigger' in el.dataset;
...
class ProductList {
  ...
  unload() {
    ...
    this.share.unload();
    this.events.unregisterAll();
    this.quickshops.forEach(quickshop => quickshop.unload());
  }
  ...
  _load() {
    ...
    this.events = new EventHandler/* default */.Z();
    this.events.register(this.el, 'focusin', e => this._loadQuickshop(e.target));
    this.events.register(this.el, 'mouseover', e => this._loadQuickshop(e.target));
    this.events.register(this.el, 'click', e => this._openQuickshop(e.target));
  }

  _getQuickshop(el) {
    let quickshop = this.quickshops.get(el);
    if (quickshop) return quickshop;
    quickshop = new ProductQuickshop({
      el,
      id: this.id,
  ...
  _loadQuickshop(target) {
    if (!isQuickShopTrigger(target)) return;
    const productItemEl = target.closest('[data-product-item-id]');
    const quickshopEl = productItemEl.querySelector('[data-product-quickshop]');

    this._getQuickshop(quickshopEl).load();
  }

  _openQuickshop(target) {
    if (!isQuickShopTrigger(target)) return;
    const productItemEl = target.closest('[data-product-item-id]');
    const quickshopEl = productItemEl.querySelector('[data-product-quickshop]');
  ...
  _processImage() {
    const unprocessedImages = this.el.querySelectorAll('.product-item-image:not(.processed)');

    for (let i = 0; i < unprocessedImages.length; i++) {
      const imageWrapper = unprocessedImages[i];
      const image = imageWrapper.querySelector('img');
      imageWrapper.classList.add('processed');

      if (image && image.getBoundingClientRect().height < 100) {
        const imageOverlay = imageWrapper.querySelector('.product-item-details-overlay');
...
;// CONCATENATED MODULE: ./source/scripts/components/Filtering.js
...
class Filtering {
  ...
  _filterClick(target) {
    ...
    const checkbox = target.querySelector('.filter-icon--checkbox .checkmark');
    const checkmark = target.querySelector('.filter-icon--checkbox .checkmark__check');

    if (target.hasAttribute('data-filter-active')) {
      checkbox.dataset.animationState = 'unchecked';
      checkmark.dataset.animationState = 'unchecked';
      target.removeAttribute('data-filter-active');
    } else {
      checkbox.dataset.animationState = 'checked';
      checkmark.dataset.animationState = 'checked';
  ...
  _initAnimations() {
    this.summaryEls.forEach(summary => {
      ...
      this.disclosureAnimations[index] = animations_es_transition({
        el: filterList,
  ...
  _setStyleVariables(el) {
    const container = this.el.querySelector('[data-faceted-filter]');
...
function updateSortFilter(e) {
  let search;
  const Sorting = {};
  Sorting.sort_by = document.querySelector('[data-sort-input]').value;
  const currentVendorSelected = e.target.closest('[data-sorting]').classList.contains('vendor-collection');
...
class Collection {
  constructor(section) {
    ...
    this.rtes = [];
    this.filtering = null;
    this.filters = this.el.querySelector('[data-faceted-filters]');
    this.sort = this.el.querySelector('[data-sorting]');
    this.sortInput = this.el.querySelector('[data-sort-input]');
    this.dataCollectionGrid = this.el.querySelector('[data-collection-grid]');

    if (this.filters || this.sort) {
      this.filtering = new Filtering(this.el);
    }
    ...
    if (section.type === 'static-collection') {
      this._initializeLoadInAnimations();
  ...
  _initializeLoadInAnimations() {
    if ('reduceAnimations' in document.body.dataset) return;
    /*
      Stop observing before re-initializing animations. Then we pass in our own threshold value.
      The observer won't fire on initial page load for most viewport sizes,
      so this will prevent that issue.
    */

    removeLoadInAnimationsAutoplay([this.dataCollectionGrid]);
    initLoadInAnimationsAutoplay({
      threshold: 0
    });
    const productsPerRow = this.data.products_per_row;
    const productItems = this.dataCollectionGrid.querySelectorAll('.product-item.pxu-lia-element');
    this.dataCollectionGrid.style.setProperty('--lia-delay', productsPerRow);

    for (let i = 0; productItems[i] && i < productsPerRow; i++) {
      productItems[i].classList.add('first-row-el');
    }
  }
  ...
  _bindEvents() {
    ...
    if (this.sortInput) {
      this.events.register(this.sortInput, 'change', updateSortFilter);
    }
  }

  _initializeInfiniteScroll() {
    // Infinite scroll
    if (this.data.pagination_method.indexOf('infinite_scroll') > -1 && this.data.products_per_page < this.data.total_products) {
        ...
        const observer = new IntersectionObserver(entries => {
          entries.forEach(entry => {
            if (entry.intersectionRatio > 0) {
              this.scrollTargetVisible = true;

              this._infiniteScroll();
            } else {
              this.scrollTargetVisible = false;
            }
          });
        });
  ...
  _infiniteScrollHandler() {
    // Use handler so _infiniteScroll isn't called with event as argument
    this._infiniteScroll();
  }

  _infiniteScroll() {
    let autoScrollCount = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;
    ...
    xhr.onload = () => {
      ...
      const products = response.querySelectorAll('[data-product-item-id]');
      const collectionGrid = document.querySelector('[data-collection-grid]');
      products.forEach(product => {
        collectionGrid.appendChild(product);
        this.productsLoadedCount++;
        this.productList.reload(); // Add a timeout to give rimg a little more time to load placeholder SVG
        // to prevent images from loading at the wrong size in Firefox

        setTimeout(() => rimg_shopify_dist_index_es.watch(product), 2);
      });

      if (this.productsLoadedCount === this.data.total_products) {
        // we've loaded the full collection
        this.fullCollectionLoaded = true;
        this.showMoreButton.style.display = 'none';
      }

      this.showMoreButton.classList.remove('loading');
      this.showMoreButton.innerHTML = this.showMoreButton.dataset.infiniteScrollShowMore; // If target is still in viewport, we want to load more items.
      // But let's keep track of how many times we've done this, and
      // take a bit of time to let IntersectionObserver catch up.

      setTimeout(() => {
        if (this.scrollTargetVisible) {
          this._infiniteScroll(autoScrollCount + 1);
        }
      }, 200);
    };
...
class DynamicBlog {
  constructor(section) {
    this.el = section.el;
    this.rtes = [];
    this.el.querySelectorAll('[data-blog-article-content]').forEach(el => this.rtes.push(new Rte({
      el
    })));
  }
...
;// CONCATENATED MODULE: ./source/scripts/sections/DynamicCountdown.js


class DynamicCountdown {
  constructor(section) {
    this.el = section.el;
    new pxs_countdown_timer_dist_index_es(this.el);

    if (section.data.productId) {
      this.productList = new ProductList({
        el: this.el,
        id: `shopify-section-${section.id}`
      });
...
;// CONCATENATED MODULE: ./source/scripts/sections/DynamicLogoList.js



class DynamicLogoList {
  constructor(section) {
    this.el = section.el;
    this.settings = section.data;
    this.events = new EventHandler/* default */.Z();
    this.carousel = this.el.querySelector('[data-logo-list-wrapper]');
    this.slides = this.el.querySelectorAll('[data-logo-list-logo]');
    this.mobile_layout = this.settings.mobile_layout;

    this.onBreakpointChange = () => {
      if (this.mobile_layout === 'slider' && cjs/* max */.Fp('S') && this.slides.length > 2) {
        this._initFlickity();
      } else {
    ...
    this.onBreakpointChange();
    cjs/* onChange */.z2(this.onBreakpointChange);
  }
  ...
  onSectionUnload() {
    ...
    cjs/* offChange */.F(this.onBreakpointChange);
    this.events.unregisterAll();
  }
...
class DynamicPromotions {
  constructor(section) {
    ...
    this.events.register(window, 'resize', just_debounce_default()(() => {
      this.blocks.forEach(block => {
  ...
  checkBlockOverflow(block) {
    ...
    if (contentHeight > imageHeight) {
      ...
      block.style.setProperty('--promotion-block-height', `${imageHeight}px`);
      block.classList.remove('promotion-block--content-overflow');
    }
  }
...
class DynamicSlideshow {
  constructor(section) {
    ...
    if (this.carousel && this.slides.length > 1) {
      /*
        To prevent Flickity from causing layout shifts when inserting the slides,
        we'll manually set the height initially.
      */
      const initialCarouselHeight = this.carousel.getBoundingClientRect().height;
      const firstSlideHeight = initialCarouselHeight ? `${initialCarouselHeight}px` : 'auto';
      this.carousel.style.setProperty('--first-slide-height', firstSlideHeight);
      /*
        Set the default speed and friction/bounciness of the slider initially.
        If it has to be set to a specific value, set it to that.
        Otherwise, set it to any value needed on mobile.
      */

      let selectedAttraction = 0.03;

      if (this.settings.slide_attraction) {
        selectedAttraction = parseFloat(this.settings.slide_attraction);
      } else if (this.viewportIsMobile()) {
        selectedAttraction = 0.15;
      }

      let friction = 0.3;

      if (this.settings.slide_friction) {
        friction = parseFloat(this.settings.slide_friction);
      } else if (this.viewportIsMobile()) {
        friction = 0.8;
      }
      ...
      this.flickity.on('change', index => {
        /*
          We need to remove `--first-slide-height` but doing this in Flickity's `ready` event
          doesn't seem to help, so we'll remove it if/when the slide is changed.
          By the time a slide change occurs, Flickity will be fully loaded and we can safely
          remove this property.
        */
        this.carousel.style.removeProperty('--first-slide-height'); // Manually play load-in animations when changing slides.

        resetLoadInAnimations(Array.from(this.slides).filter((el, i) => i !== index));
  ...
  _positionNavigation() {
    if (this.settings.mobile_navigation_adjust) {
      ...
      const currentSlideheight = this.flickity.selectedElement.querySelector('[data-slide-image-wrapper]').offsetHeight;
      ...
      if (this.pagination) {
        ...
        if (this.viewportIsMobile()) {
          ...
          if (content) {
            ...
            const paginationHeight = this.pagination.offsetHeight; // If no content, and mobile, offset by height of dots, and the bottom gutter
  ...
  _updateCounter(index) {
    const counterText = this.counter.dataset.counterTemplate.replace('{{ count }}', index).replace('{{ total }}', this.slides.length);
...
class DynamicTestimonials {
  constructor(section) {
    this.el = section.el;
    this.events = new EventHandler/* default */.Z();
    this.testimonials = this.el.querySelectorAll('[data-testimonial]');
    this.sliderEl = this.el.querySelector('[data-testimonials-slider]');
    this.flickity = null;

    if (this.checkSliderCondition()) {
      this.initFlickity();
    }
  ...
  checkSliderCondition() {
    const desktopSliderThreshold = 4;
    const mobileSliderThreshold = 2;
    return this.testimonials.length >= desktopSliderThreshold || this.testimonials.length >= mobileSliderThreshold && cjs/* max */.Fp('S');
  }

  initFlickity() {
    this.flickity = new (js_default())(this.sliderEl, {
      accessibility: false,
      // Flickity's inherent "accessibility" actually makes it worse
      adaptiveHeight: false,
      cellAlign: 'center',
...
class StaticAddresses {
  constructor(section) {
    ...
    script_default()([scripts.dataset.shopifyCommonUrl], () => script_default()([scripts.dataset.customerAreaUrl], () => this._addAddressObservers()));
  }

  _addNewToggles() {
    this.newToggles = this.el.querySelectorAll('[data-customer-toggle-new-form]');

    for (let i = 0; i < this.newToggles.length; i++) {
      this.newToggles[i].addEventListener('click', newToggleHandler);
    }
  }

  _addToggles() {
    const toggleEls = this.el.querySelectorAll('[data-customer-toggle-form]');
    this.toggles = [];
    ...
    for (let i = 0; i < toggleEls.length; i++) {
      ...
      toggleEls[i].addEventListener('click', this.toggles[i].handler);
    }
  }

  _addDeletes() {
    const deleteEls = this.el.querySelectorAll('[data-customer-delete-form]');
    this.deletes = [];
    ...
    for (let i = 0; i < deleteEls.length; i++) {
      ...
      this.deletes[i].handler = e => {
        e.preventDefault();
        window.Shopify.CustomerAddress.destroy(deleteEls[i].dataset.customerDeleteForm, this.data.message_confirm_delete);
      };

      deleteEls[i].addEventListener('click', this.deletes[i].handler);
    }
  }

  _getAddressIDs() {
    const addressIDEls = this.el.querySelectorAll('[data-address-id]');
    const addressIDs = [];
    addressIDEls.forEach(addressIDEl => addressIDs.push(addressIDEl.dataset.addressId));
    return addressIDs;
  }

  _addAddressObservers() {
    ...
    this.addressIDs.forEach(id => {
      this.addressObservers.push(new window.Shopify.CountryProvinceSelector(`address-country-${id}`, `address-province-${id}`, {
  ...
  onSectionUnload() {
    ...
    this.toggles.forEach(toggle => toggle.el.removeEventListener('click', toggle.handler));
    this.deletes.forEach(del => del.el.removeEventListener('click', del.handler));
    this.selects.forEach(select => select.unload());
  }
...
class StaticBlog {
  constructor(section) {
    this.el = section.el;
    this.dataBlogPostsGrid = this.el.querySelector('[data-blog-posts-grid]');
    this.rtes = [];
    this.el.querySelectorAll('[data-blog-article-content]').forEach(el => this.rtes.push(new Rte({
      el
    })));
  ...
  _initializeLoadInAnimations() {
    removeLoadInAnimationsAutoplay([this.dataBlogPostsGrid]);
    initLoadInAnimationsAutoplay({
      threshold: 0
    });
  }
...
class StaticCart {
  constructor(section) {
    ...
    this.el = section.el;
    this.updateTimeout = null;
    this.dataErrorMsg = this.el.querySelector('[data-error-msg]');

    this._bindEvents();
  }

  _bindEvents() {
    const quantityInputs = this.el.querySelectorAll('[data-quantity-key]');
    const removeItemButtons = this.el.querySelectorAll('[data-remove]');
    quantityInputs.forEach(el => this.events.register(el, 'input', event => this._updateItemQuantity(event)));
    removeItemButtons.forEach(el => this.events.register(el, 'click', event => this._updateItemQuantity(event)));
  }
  ...
  _updateItemQuantity(event) {
    ...
    this.updateTimeout = setTimeout(() => {
      ...
      /*
        When a non-integer value is used, Shopify throws an alert which doesn't have
        a user-friendly error message, so we'll handle this ourselves.
      */

      if (!Number.isInteger(Number(quantity))) {
        this.dataErrorMsg.classList.remove('hidden');
        return;
      }

      window.Shopify.changeItem(lineItemKey, quantity, cart => {
  ...
  updateCart(cart, timeoutId) {
    ...
    shopify_asyncview_dist_index_es.load(window.Theme.routes.cart_url, {
      ...
      this._unbindEvents(); // Inject new cart table contents


      const newTableContainer = document.createElement('div');
      newTableContainer.innerHTML = html.table;
      const cartTable = morphdom_esm(this.el.querySelector('[data-cart-table]'), newTableContainer.querySelector('table'), {
      ...
      const cartDiscountContainer = this.el.querySelector('[data-cart-discount-container]');
      const totalPrice = this.el.querySelector('[data-total-price]');
      const cartItemCountWrap = document.querySelector('[data-header-cart-count]'); // Inject new cart level discounts
      ...
      if (cartItemCountWrap) {
        cartItemCountWrap.innerText = `(${data.itemCount})`;
      }
...
class StaticGiftCard {
  constructor(section) {
    const scripts = document.querySelector('[data-scripts]');
    const cardCodeEl = section.el.querySelector('[data-gift-card-code]');
    const qrCodeEl = section.el.querySelector('[data-gift-card-qr-code]');

    if (cardCodeEl) {
      cardCodeEl.addEventListener('click', () => selectText(cardCodeEl));
    }

    if (qrCodeEl) {
      script_default()([scripts.dataset.qrCodeJsUrl], () => initializeQRCode(qrCodeEl));
    }
  }

}
;// CONCATENATED MODULE: ./source/scripts/components/SiteOverlay.js
...
;// CONCATENATED MODULE: ./source/scripts/components/SearchFlyout.js
...
class SearchFlyout {
  constructor(el, settings) {
    ...
    this.settings = settings;
    this.events = new EventHandler/* default */.Z();
    this.siteOverlay = new SiteOverlay();
    this.header = document.querySelector('[data-site-header]');
    this.headerContainer = document.querySelector('[data-site-header-container]');
    this.headerControls = document.querySelector('[data-header-controls]');
    this.input = this.el.querySelector('[data-search-input]');
    this.close = this.el.querySelector('[data-search-flyout-close]');
    this.formOffset = this.close.offsetWidth;
    this.animation = animations_es_transition({
      el: this.el,
      state: 'closed'
    });
    this.isOpen = false;
    this.events.register(window, 'keyup', e => this._onKeyUp(e));
    this.events.register(this.close, 'click', () => this._close());
    this.events.register(this.close, 'blur', () => this._close());
    cjs/* onChange */.z2(() => this._onResize());
    this.events.register(document.body, 'click', e => {
      if (!e.target.closest('[data-search-trigger], [data-search-flyout]')) {
        this._close();
      }
    });
    ...
    if (this.settings.sticky_header === 'true' && cjs/* min */.VV('L')) {
      this._onSticky();
    } // Offset the search form based on the width of the close button


    this.el.style.setProperty('--search-form-offset', `${this.formOffset}px`);
  }
  ...
  _onSticky() {
    /*
      Loop through the sticky header elements to determine the highest maximum height,
      apply the highest maximum height to the search flyout container.
    */
    const stickyElements = document.querySelectorAll('[data-sticky-element]');
    const stickyElementHeights = Array.from(stickyElements).map(el => el.offsetHeight);
    const searchFlyoutHeight = Math.max(...stickyElementHeights);
    this.el.style.setProperty('--search-flyout-height', `${searchFlyoutHeight}px`);
  }

  _open() {
    if (this.isOpen) return;
    this.isOpen = true;
    this.animation.animateTo('open').then(() => {
      const {
        length
      } = this.input.value;
      ...
      this.siteOverlay.open();
    });
  }

  _close() {
    if (!this.isOpen) return;
    this.animation.animateTo('closed').then(() => {
      this.isOpen = false;
      ScrollLock.unlock();
      this.siteOverlay.close();

      if (cjs/* min */.VV('L')) {
        // Show the header controls when the search flyout is closed
        this.headerControls.style.zIndex = '901';
      }
    });
  }
...
class NavDesktopParent {
  constructor(el, options) {
    ...
    this.click = e => {
      e.stopPropagation();
      const clickedParent = e.target.closest('[data-nav-desktop-link]'); // If the link being clicked is a parent link, then take control of the navigation
...
class NavMobile {
  constructor(_ref) {
    ...
    this.events.register(this.overlay, 'click', () => this.close());
    this.events.register(window, 'keydown', e => this.closeEsc(e));
    this.submenuTriggers.forEach(submenuTrigger => {
      this.events.register(submenuTrigger, 'click', e => this.openSubmenu(e));
    });
  }
...
class PredictiveSearch {
  constructor(el) {
    ...
    this.events = new EventHandler/* default */.Z();
    this.input = this.el.querySelector('[data-search-input]');
    this.results = this.el.querySelector('[data-search-results]');
    this.resultsContainer = this.el.querySelector('[data-search-results-container]');
    this.animation = animations_es_transition({
      el: this.resultsContainer
    });
    this.isOpen = false;
    this.events.register(window, 'keyup', e => this._onKeyUp(e));
    this.events.register(window, 'resize', just_debounce_default()(() => this._onResize(), 250));
    this.events.register(this.el, 'submit', e => this._onSubmit(e));
    this.events.register(this.input, 'input', just_debounce_default()(() => this._onChange(), 250));
    this.events.register(this.input, 'focus', () => this._onFocus());
    this.events.register(this.input, 'keydown', e => this._onKeydown(e));
    this.events.register(document.body, 'click', e => {
  ...
  _onSubmit(e) {
    if (!this._getQuery().length) e.preventDefault();
  }
  ...
  _getSearchResults(searchTerm) {
    this.resultsContainer.setAttribute('data-loading', 'true');
    fetch(`${window.Theme.routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&section_id=predictive-search`).then(response => {
      ...
      rimg_shopify_dist_index_es.watch(this.results);
    }).catch(error => {
      this._close();
  ...
  _renderSearchResults(resultsMarkup) {
    this.resultsContainer.setAttribute('data-loading', 'false');
  ...
  _registerEvents() {
    this.searchButton = this.el.querySelector('[data-predictive-search-button]');
    this.events.register(this.searchButton, 'blur', () => this._onBlur());
  }

  _resizeSearchResults() {
    const offset = 30;
    const position = this.input.getBoundingClientRect();
    const maxHeight = window.innerHeight - position.bottom - offset;
    this.resultsContainer.style.setProperty('--results-container-height', `${maxHeight}px`);
  }
  ...
  _open() {
    ...
    this.isOpen = true;
    this.animation.animateTo('open').then(() => {
      this.input.classList.add('search-form__input--active');
    });
  }
  ...
  _close() {
    ...
    this.animation.animateTo('closed').then(() => {
      this.isOpen = false;
      this.input.classList.remove('search-form__input--active');
    });
  }
...
class StaticHeader {
  constructor(section) {
    ...
    this.headerContainer = this.el.querySelector('[data-site-header-container]');
    this.siteOverlay = document.querySelector('[data-site-overlay]');
    this.logo = this.el.querySelector('[data-header-logo]');
    this.sticky = this.data.sticky_header;
    this.search = this.el.querySelector('[data-search-trigger]');
    this.searchForm = this.el.querySelector('[data-search-form]');
    this.searchFlyoutEl = this.el.querySelector('[data-search-flyout]');
    this.searchFlyout = new SearchFlyout(this.searchFlyoutEl, this.data);
    this.enablePredictiveSearch = this.data.enable_predictive_search;
    ...
    const fadeHeight = Math.max(this.logo.getBoundingClientRect().height + barHeight - 10, 100); // Only set the property when the header container is below the announcement bar

    if (this.headerContainer.offsetTop > this.announcementBar.offsetTop) {
      document.documentElement.style.setProperty('--announcement-bar-height', `${barHeight}px`);
    }

    if (this.enablePredictiveSearch === 'true') {
      this.predictiveSearch = new PredictiveSearch(this.searchForm);
    }
    ...
    this.events.register(window, 'scroll', () => {
      ...
      const searchBottom = this.search.getBoundingClientRect().bottom;
      const navigationTop = this.navigation.getBoundingClientRect().top;
      const siteHeaderTop = this.header.getBoundingClientRect().top;
      const scrollHitbox = cjs/* max */.Fp('M') ? siteHeaderTop : navigationTop;

      if (this.sticky === 'true') {
        if (searchBottom > scrollHitbox) {
          this.searchFlyoutEl.classList.add('search-flyout--sticky');
        } else {
          this.searchFlyoutEl.classList.remove('search-flyout--sticky');
        }
      }
    });
    this.events.register(this.search, 'click', () => {
      if (cjs/* min */.VV('L')) {
        // Hide the header controls when the search flyout is shown
        this.headerControls.style.zIndex = '900';
      }

      this.searchFlyout._open();
    });
  }
...
class StaticPassword {
  constructor(_ref) {
    ...
    this.el = el;
    this.modal = new Modal();
    this.events.register(document.querySelector('[data-open-modal-button]'), 'click', () => this.openModal());
...
class StaticProductRecommendations {
  ...
  _loadRecommendations(container, productId, limit) {
    const url = `${window.Theme.routes.product_recommendations_url}?section_id=${this.sectionId}&limit=${limit}&product_id=${productId}`;
    shopify_asyncview_dist_index_es.load(url, {
      ...
      this.productList = new ProductList({
        el: container,
        id: `shopify-section-${this.sectionId}`
      });
...
;// CONCATENATED MODULE: ./source/scripts/sections/Search.js
...
const scripts = document.querySelector('[data-scripts]');
script_default()([scripts.dataset.shopifyApiUrl], () => {
  initLoadInAnimationsAutoplay({
    threshold: 0.1
  });
  rimg_shopify_dist_index_es.init();
  const sections = new shopify_sections_manager_es();
  sections.register('dynamic-featured-collection', section => new Collection(section));
  sections.register('dynamic-featured-product', section => new Product(section));
  sections.register('dynamic-blog', section => new DynamicBlog(section));
  sections.register('dynamic-countdown', section => new DynamicCountdown(section));
  sections.register('dynamic-logo-list', section => new DynamicLogoList(section));
  sections.register('dynamic-promotions', section => new DynamicPromotions(section));
  ...
  sections.register('static-article', section => new Share(section));
  sections.register('static-blog', section => new StaticBlog(section));
  sections.register('static-cart', section => new StaticCart(section));
  sections.register('static-collection', section => new Collection(section));
  sections.register('static-contact-page', section => new StaticPage(section));
  ...
  sections.register('static-product', section => new Product(section));
  sections.register('static-product-recommendations', section => new StaticProductRecommendations(section));
  sections.register('static-quickshop', section => new Product(section));
  sections.register('search', section => new Search(section));
...
/******/ })()
;
//# sourceMappingURL=editions.js.map?1688580287170
